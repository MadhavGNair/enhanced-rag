...
async def evaluate_results(
    ...
    evaluator = Evaluator(question, response, retrieved_contexts, groundtruth_context)
...
if __name__ == "__main__":
    ...
    response = vanilla_rag(pdf_path, question)

    evaluation_results = asyncio.run(
        evaluate_results(
            question, groundtruth_answer, groundtruth_context, response, "vanilla_rag"
        )
    )

    # add evaluation results to the response dictionary
